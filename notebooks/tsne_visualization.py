"""
t-SNE Visualization — Global Before/After oversampling, shared embedding.

Generates 2 PNGs (Global only — partition schemes only affect per-client data,
so the global view is identical across IID/Dirichlet variants):
  - tsne_global_smote_1to1.png
  - tsne_global_adasyn_1to1.png

Each PNG shows:
  - 1 row x 2 columns (Before / After oversampling)
  - t-SNE is fit ONCE per PNG on (original subsample + synthetic-only rows),
    then split by index. SMOTE/ADASYN return original rows first, synthetic
    appended after, so original points keep identical coordinates in both
    panels and After only differs from Before by the added red dots.
  - Per-panel data: all fraud + N_NONFRAUD non-fraud (no fraud cap).
  - Non-fraud: blue, Fraud: red.

Output: results/visualizations/tsne_global_{smote,adasyn}_1to1.png
"""
...
def main() -> None:
    ...
    runs = [
        (
            "smote",
            os.path.join(out_dir, f"tsne_global_smote_{RATIO_TAG}.png"),
            f"t-SNE Visualization — Global | SMOTE [ratio={ratio_label}] {sub_note}",
        ),
        (
            "adasyn",
            os.path.join(out_dir, f"tsne_global_adasyn_{RATIO_TAG}.png"),
