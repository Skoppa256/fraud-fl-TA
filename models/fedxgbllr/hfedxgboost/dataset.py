...
def load_single_dataset(
    ...
    # PaySim fast path — preprocessing already done by shared pipeline
    if isinstance(datafiles_paths, tuple) and datafiles_paths[0] == "__paysim__":
        _, x_train, y_train, x_test, y_test = datafiles_paths
        x_train = x_train.astype(np.float32)
        x_test = x_test.astype(np.float32)
        # Flower expects float for BCE
        ...
        print(
            "Fraud ratio in train data:",
            float((y_train != 0.0).sum()) / x_train.shape[0],
        )
...
def divide_dataset_between_clients(
    ...
    """Divide the data between clients (IID or Dirichlet non-IID).
    ...
    ----------
        ...
        non_iid_alpha (float, optional): If provided, use Dirichlet(alpha)
            partitioning instead of IID.
        random_state (int): Seed for the partitioning RNG.
    ...
    if non_iid_alpha is not None:
        # Non-IID Dirichlet partition via shared module
        import sys
        import os
        ...
        x_np = trainset.tensors[0].numpy()
        y_np = trainset.tensors[1].numpy().astype(np.int32)
        client_dicts = get_partition(
            x_np, y_np,
            scheme='dirichlet',
            alpha=non_iid_alpha,
        ...
        datasets = [
            TensorDataset(
                torch.from_numpy(cd['x'].astype(np.float32)),
                torch.from_numpy(cd['y'].astype(np.float32)),
            )
            for cd in client_dicts
        ]
        print(
            f"[dirichlet] non-IID partition with alpha={non_iid_alpha} — "
            f"client sizes: {[len(d) for d in datasets]}"
        )
    else:
        # IID random_split path
