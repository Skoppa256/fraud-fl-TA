...
def test_extended(
...
) -> Tuple[float, float, int, NDArray, NDArray, NDArray]:
    """Same as ``test`` but also collects per-sample probs and labels.

    Returns a 6-tuple ``(avg_loss, avg_metric, n_samples, probs, preds, y_true)``
    where ``probs`` are raw model outputs (sigmoid in BINARY), ``preds`` are
    0/1 predictions thresholded at 0.5, and ``y_true`` are the ground-truth
    labels. Arrays are 1-D numpy.
    """
    ...
    with torch.no_grad():
        for data in tqdm(testloader, desc="TEST") if log_progress else testloader:
            ...
            probs_list.append(outputs.detach().cpu().numpy().reshape(-1))
    ...
    y_true = np.concatenate(labels_list) if labels_list else np.array([])
    preds = (probs >= 0.5).astype(int)
    return total_loss / n_samples, total_result / n_samples, n_samples, probs, preds, y_true
