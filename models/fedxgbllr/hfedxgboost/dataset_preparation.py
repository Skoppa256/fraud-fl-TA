...
def _load_paysim() -> tuple:
    """Load PaySim dataset via the shared preprocessing pipeline.

    Uses preprocessing.paysim.load_paysim() which performs:
      1. Drop: nameOrig, nameDest, isFlaggedFraud
      2. Feature engineering: errorBalanceOrig, errorBalanceDest
      3. One-hot encoding: type column (5 categories)
      4. StandardScaler: fit on train only (no leakage)
      5. Stratified 70/15/15 split (seed=42)

    Returns
    -------
        (x_train, y_train, x_test, y_test) as float32/int32 numpy arrays.
        Val set (15%) is stored in _PAYSIM_VAL_CACHE for serverside_eval.
    """
    global _PAYSIM_VAL_CACHE
    from preprocessing.paysim import load_paysim
    data = load_paysim(
        data_path=_os.path.join(_PROJECT_ROOT, 'data', 'paysim', 'paysim.csv')
    )
    _PAYSIM_VAL_CACHE = (data['x_val'], data['y_val'])
    return (
