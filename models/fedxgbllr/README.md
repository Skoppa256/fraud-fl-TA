...
These steps assume you have `conda` installed (Miniconda or Anaconda) and a Python 3.10.x build available via conda's default channel. They reproduce the same environment as the Poetry instructions above, but using a self-contained conda environment named `hfedxgboost` and installing all third-party dependencies with `pip` so the pins from `pyproject.toml` are honoured exactly.

**1- Prerequisites**
- `conda` available on your `PATH` (verified with `conda --version` — tested with conda `26.1.1`).
- A Python `3.10.*` build available on conda's default channel (verified with `conda search "python=3.10.*"` — any of `3.10.6`–`3.10.20` works; the rest of this guide uses `3.10.14`).

**2- Create the conda environment**

From the directory that contains `pyproject.toml` (i.e. `/baselines/hfedxgboost`), run:
```bash
# Create a fresh env with the right interpreter and a recent pip
conda create -n hfedxgboost python=3.10.14 pip -y
```

**3- Activate the environment**
```bash
conda activate hfedxgboost
```

**4- Install the baseline's dependencies via pip**

These mirror the pins in `pyproject.toml`. `torchvision` is omitted because it is not imported anywhere in the source. We also pin `setuptools<81` because Ray (a transitive dependency of `flwr[simulation]==1.5.0`) still imports `pkg_resources`, which was removed in setuptools 81.
```bash
...
**5- Install the PaySim extras**

These are not used by the original baseline, but are required by the PaySim/non-IID/SMOTE/SHAP extensions documented elsewhere in this repository.
```bash
pip install \
  "pandas==2.2.2" \
...
So that `python -m hfedxgboost.main ...` picks up local edits without re-installing:
```bash
pip install -e .
```
...
A short federated run on the smallest binary-classification dataset (a9a, 2 clients, 2 rounds) is a good way to confirm everything is wired up — the dataset auto-downloads from the LIBSVM repository on first use:
```bash
python -m hfedxgboost.main dataset=a9a clients=a9a_2_clients run_experiment.num_rounds=2 use_wandb=true
```
A successful smoke test prints the resolved Hydra config, an `FL starting` banner, two rounds of `fit progress: (...)` lines, and a final `History` object.
