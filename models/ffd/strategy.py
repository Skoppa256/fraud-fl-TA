"""AccuracyWeightedFedAvg — FFD aggregation strategy (Yang et al., 2019).
...
where:
    n_c  = number of training samples on client c (after local oversampling)
    n    = total samples across all clients
    α_c  = local AUPRC of client c (reported in fit metrics)
    w_c  = client c's local model weights

This gives higher influence to clients with more data AND better local
performance. When all α_c == 0 (round 0 cold start, where the local model
hasn't trained from a non-trivial init) the formula collapses; we fall back
to standard FedAvg (weight by n_c/n only) so the aggregation always
produces well-defined parameters.
"""
...
class AccuracyWeightedFedAvg(fl.server.strategy.FedAvg):
    """FedAvg variant weighted by both data size and local AUPRC.

    Only :meth:`aggregate_fit` is overridden; all other behaviour
    (``configure_fit``, ``evaluate_fn``, parameter init, ...) is inherited
    from the standard FedAvg implementation.
    """
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        # weighted sum of each parameter layer across all clients
        ...
        print(f"[FFD] round {server_round} aggregation weights"
...
def get_strategy(
    ...
    """Build :class:`AccuracyWeightedFedAvg` with centralized server-side eval."""
