"""Centralized baseline — Gradient Boosting Machine (HistGBM).

Same model class and hyperparameters as models/gbm_bestmodel/client.py.
No Flower, no client splitting, no aggregation.

Methodological note:
    Identical HistGradientBoostingClassifier(max_iter=100,
    learning_rate=0.1, max_depth=6) as the federated GBM.
    In the federated setting only one client's model is selected
    as global per round (best-model selection). Here, the model
    trains on the full dataset — this is the upper bound for
    GBM-based FL.
