...
#
# UNIFORM RULE: sampling_strategy=0.01 for BAF, identical to PaySim/ULB — this
# study holds the PARAMETER constant across all three datasets rather than tuning
# per dataset. Consequence on BAF: global prevalence (~1.10%) exceeds the 1:100
# target, so under IID every client skips via `target_met` (SMOTE and no-SMOTE
# arms become identical); only under Dirichlet skew do starved clients fall below
# the target and oversample. That selectively isolates the intervention onto the
# sparse partitions — a cleaner experiment than uniform oversampling. The value is
# pinned here and passed explicitly to ALL SIX models (argparse
# --sampling_strategy AND Hydra dataset.oversampling.sampling_strategy) so no
# model silently falls back to a per-config default. NB: the bare per-model
# run_*.sh also default to 0.01, so they now agree with this driver for BAF.
#
# KNOWN IDENTITY (per-client census): on BAF, the IID SMOTE arm and the
# centralized SMOTE arm are bit-identical to their no-SMOTE counterparts — every
# client/global set already exceeds the 1:100 target so SMOTE is skipped
# (15/15 clients via target_met). Those two SMOTE cells can be skipped and
# reported as identical to `none` rather than recomputed. Dirichlet arms still
# oversample the sparse clients and must be run.
#
...
# Runs sequentially — BAF is large (1M rows), so no parallelism. Logs land in
...
#   DRYRUN=1 bash experiments/run_all_centralized_iid_dirichlet1.0_baf.sh
